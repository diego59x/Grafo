...
| GrafoMatrixAdyacencia.java													   |
| Fecha de creacion: 24/11/2017													   |
|                                                                                  |
* @author raidentrance															   |
| Tomado de https://devs4j.com/2017/11/24/implementa-un-grafo-de-ciudades-en-java/ | 
...
public void insertaArista(int v1, int v2, int dist)
	...
	if (v1 >= MAXIMO_VERTICES || v2 >= MAXIMO_VERTICES) {
		throw new ArrayIndexOutOfBoundsException("Vertices inválidos, fuera de rango" + "nRango de vertices: 0 - " + (getMAX_VERTICES() - 1));
...
*/
// Este si lo hice yo :D
public int tomarValor(int v1, int v2){
	return matrix[v1][v2];
}
